...
export let parseCreatures = async (textInput, urlInput, sourceInput) => {
  ...
  let getCreatureText = (creatureRegExp, sourceTextMod) => {
    ...
    if (regArr.indexOf(creatureRegExp) == listOfCreatures.length - 1) {
      return sourceTextMod.slice(start, sourceTextMod.search("Rarities\n"));
    }
  ...
  let getDescription = (text, reg) => {
    ...
    return text
      .slice(text.search(descStartReg), text.search(attributesReg))
      .trim();
  };
  ...
  let getSkills = (txt) => {
    ...
    workText = workText.slice(
      workText.search(/SKILLS/),
      workText.search(/TALENTS|MOVEMENT|ARMOR/)
    );
    //RegExp to match skill name
    let skillsNameReg = /(?<=SKILLS:|,)\s*\w+\b/gm;
    ...
    for (let i = 0; i < skillsNameArr.length; i++) {
      workText = workText.slice(
        workText.search(new RegExp(skillsNameArr[i], "i"))
      );
  ...
  let getTalents = (txt) => {
    ...
    let talentsNameReg = /(?<=TALENTS:|,)[\s\w+]*(?=\d)/gm;
  ...
  let getImg = async (name) => {
    let path = filesArr.find((e) => e.match(`${name}.png`));
    if (!path) {
      path = defaultImgPath;
  ...
  let getTokenImg = async (name) => {
    let tokenPath = filesArr.find((e) => e.match(`${name} Token.png`));
    if (!tokenPath) {
      tokenPath = defaultImgPath;
      console.log(`Image of ${name} not found. Using default img`);
    }
    return tokenPath;
  };
  ...
  let getAttacks = async (txt) => {
    ...
    attackArr = attackArr.map((e) => {
      ...
      attackData.data.description = e.slice(
        e.search(attackData.name) + attackData.name.length
      );
      if (e.search(/Damage \d/i) != -1) {
        attackData.data.damage = e.match(/(?<=Damage )\d/)[0];
      }
      if (e.search(/\d+(?= Base Dice)/i) != -1)
        attackData.data.dice = e.match(/\d+(?= Base Dice)/)[0];
